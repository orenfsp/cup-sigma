        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          {submissionSuccess ? (
            ...
            <form onSubmit={handleSubmitAppeal} className="space-y-6">
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-3">
                  2. Как тебе удобнее описать ситуацию?
                </label>
                ...
                <div className="space-y-3.5 text-xs">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {["В классе", "В раздевалке/коридоре", "В интернете / Telegram", "По дороге домой", "Дома"].map((opt) => (
                        <button
                          key={opt}
                          type="button"
              ...
              {/* Step 6: Crisis / Emergency Contact (Optional!) */}
              <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
                <div className="flex items-start gap-3">
                  <input
                    type="checkbox"
                    id="emergency-check"
                    checked={wantsEmergencyContact}
                    onChange={(e) => setWantsEmergencyContact(e.target.checked)}
                    className="mt-1 rounded text-teal-600 focus:ring-teal-500 w-4 h-4"
                  />
                  <div>
                    <label htmlFor="emergency-check" className="font-bold text-xs sm:text-sm text-slate-900 cursor-pointer">
                      {t(
                        "Оставить контакт для экстренной связи оператору (по желанию)",
                        "Указать контакт для экстренной связи с оператором (добровольно)"
                      )}
                    </label>
                    <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">
                      {t(
                        "Если ты хочешь, чтобы оператор при острой опасности мог экстренно связаться с тобой (Telegram или телефон). Отказ не мешает отправке — платформа остаётся на 100% анонимной.",
                        "Используется исключительно при признаках непосредственной угрозы жизни и здоровью. Хранится изолированно и доступно только кризисному оператору."
                      )}
                    </p>
                  </div>
                </div>
                ...
                {wantsEmergencyContact && (
                  <div className="mt-3 pt-3 border-t border-slate-100 animate-fadeIn">
                    <input
                      ...
                      className="w-full p-2.5 rounded-xl border border-slate-300 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
                    />
                  </div>
                )}
              </div>
